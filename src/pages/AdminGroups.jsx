...
const label = { fontSize: 'var(--text-xs)', fontWeight: 600, color: '#5f6b7a', display: 'block', marginBottom: '2px' };
const field = { width: '100%', padding: '8px 10px', borderRadius: '6px', border: '1px solid #d8dde2', fontSize: 'var(--text-sm)' };
    ...
    <AdminLayout>
      <div className="admin-page" style={{ padding: '0 32px 32px' }}>
        <div className="admin-page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px 0', borderBottom: '1px solid var(--band-line)' }}>
          <div>
            <h1 style={{ fontSize: 'var(--text-2xl)', fontWeight: 600, color: '#16232e' }}>Shipments &amp; Groups</h1>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {vehicleGroupsLoading ? (
                <tr><td colSpan={7} style={{ padding: '24px', textAlign: 'center', color: '#5f6b7a' }}>Loading groups…</td></tr>
              ) : vehicleGroups.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ padding: '28px 24px', textAlign: 'center', color: '#5f6b7a' }}>
                ...
                <tr key={g.id}>
                  <td>
                    <div style={{ fontWeight: 600, color: '#16232e' }}>{g.name}</div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: '#5f6b7a' }}>{g.id}</div>
                    {g.note && <div style={{ fontSize: 'var(--text-xs)', color: '#5f6b7a', marginTop: '3px' }}>{g.note}</div>}
                  </td>
                  <td><span className="badge" style={{ background: '#e6eff2', color: 'var(--primary-ink)' }}>{g.type || '—'}</span></td>
                  <td style={{ color: '#5f6b7a' }}>{g.vessel || '—'}</td>
                  <td style={{ color: '#5f6b7a' }}>{g.origin || '—'}</td>
                  <td style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', color: '#5f6b7a' }}>{g.arrived || '—'}</td>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '12px' }}>
                      ...
                      {!mayWrite && <span style={{ fontSize: 'var(--text-sm)', color: '#5f6b7a' }}>View only</span>}
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="modal-overlay" onClick={() => setConfirming(null)} role="dialog" aria-modal="true" aria-label="Remove group">
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ padding: '24px', maxWidth: '440px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => confirmRemove(confirming)} className="btn-primary" style={{ background: '#a13f3f', color: '#fff' }}>Remove group</button>
        ...
        <div className="modal-overlay" onClick={close} ref={trapRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={draft.id ? 'Edit group' : 'Add group'}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ padding: '24px', maxWidth: '540px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 'var(--text-2xl)', color: '#16232e' }}>
                {draft.id ? 'Edit group' : 'Add group'}
              </h3>
              <button onClick={close} aria-label="Close" style={{ border: 'none', background: '#edf1f6', borderRadius: '50%', width: '32px', height: '32px', cursor: 'pointer' }}>
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {draft.id && (
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: '#5f6b7a' }}>
